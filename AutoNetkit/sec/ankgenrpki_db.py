...
import rpki.sql_schemas, subprocess
...
def sql_create_db(cfg, rootdb, name):
  """
...
def sql_dump(cfg, name, path):
  database = cfg.get("sql-database", section = name)
  username = cfg.get("sql-username", section = name)
  password = cfg.get("sql-password", section = name)

  print "dumping database", database
  fn = path + "/" + database + ".dump"
  dumpcmd = ("mysqldump", "-u%s"%username,  "-p%s"%password, database)
  print "Running ", " ".join(dumpcmd)
  f = open(fn, "w")
  subprocess.check_call(dumpcmd,cwd=path,stdout=f)  
  

def sql_dump_dbs(cfg_file, path):
  cfg = rpki.config.parser(cfg_file,"myrpki")
  if cfg.getboolean("start_irdbd", False):
    sql_dump(cfg, "irdbd", path)

  if cfg.getboolean("start_rpkid", False):
    sql_dump(cfg, "rpkid", path)

  if cfg.getboolean("start_pubd",  False):
    sql_dump(cfg, "pubd", path)


def sql_create_dbs(cfg_file, rootpw):
  cfg = rpki.config.parser(cfg_file, "myrpki")
  rootdb = MySQLdb.connect(db = "mysql", user = "root", passwd = rootpw)
  print "*********", cfg_file
