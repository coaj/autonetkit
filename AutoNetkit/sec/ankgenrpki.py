...
$Id: yamltest.py 4433 2012-04-17 21:44:22Z sra $
...
import ankgenrpki_db

# Pathnames for various things we need
deploy_confdir = "/opt/rpki/ank"
rpki_conf = "/opt/rpki/ank-input/rpki.conf"
deploy_prefix = "/opt/rpki"  # from ./configure on the RPKI trunk
sql_rootpw = "fnord"
...
test_dir  = cleanpath(deploy_confdir)
rpkid_dir = cleanpath(deploy_prefix, "sbin")
...
class allocation(object):
  ...
  def dump_conf(self, fn):
    ...
    r = { "handle"              : self.name,
          ...
          "pubd_server_host"    : self.pubd.name, # "localhost",
          "pubd_server_port"    : str(self.pubd.pubd_port),
          "publication_rsync_server" : "%s:%s" % (self.pubd.name, self.pubd.rsync_port), # "localhost:%s" % self.pubd.rsync_port,
...
opts, argv = getopt.getopt(sys.argv[1:], "c:hkp:si?", ["config=", "help", "keep_going", "pidfile=", "skip_config", "iwp"])
for o, a in opts:
  if o in ("-h", "--help", "-?"):
    print __doc__
    sys.exit(0)
...
if len(argv) == 0:
  raise rpki.exceptions.CommandParseFailure, "Yaml Filename required"

if iwp_mode:
  print "Running in IWP mode for testing\n"
...
try:
  ...
  for d in db:
    if not d.is_hosted:
      ...
      print "Creating Databases for", d.name
      ankgenrpki_db.sql_create_dbs(d.path("rpki.conf"), sql_rootpw)
  ...
  if not iwp_mode:
    ...
    try :
      ...
      for d in db:
        ankgenrpki_db.sql_dump_dbs(d.path("rpki.conf"),d.path())
